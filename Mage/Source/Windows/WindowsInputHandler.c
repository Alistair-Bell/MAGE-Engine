...
LRESULT CALLBACK MageInputHandlerWindowsEventListener(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		...
		case WM_KEYDOWN:
		{
			...
			StaticHandler->KeyboardStates[MageInputHandlerTranslateKeyCodes((I64)wParam)] = MAGE_BUTTON_STATE_PRESS;
			return MageFalse;
		}
		case WM_SYSKEYUP:
		{
			printf("Inform: Keyrelease %d\n", (I32)wParam);
			StaticHandler->KeyboardStates[MageInputHandlerTranslateKeyCodes((I64)wParam)] = MAGE_BUTTON_STATE_RELEASE;
			return MageFalse;
		}
...
MageMouseButton MageInputHandlerTranslateMouseCode(const U64 code)
{
	switch (code)
	{
