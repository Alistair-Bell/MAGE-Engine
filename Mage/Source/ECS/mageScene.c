...
void mageSceneCreate(struct mageScene *scene, const char *sceneTag, const struct mageHeapAllocater *allocater)
{
    if (allocater == NULL)
    {
        MAGE_LOG_CORE_INFORM("Using default scene allocaters, none specified\n", NULL);
        scene->Allocater = mageHeapAllocaterDefault();
    }
    else
    {
    ...
    scene->Pool.QueueCount                  = UINT32_MAX;    
...
void mageSceneDisplayInformation(const struct mageScene *scene)
{
    MAGE_LOG_CORE_INFORM("Scene: %s (%p) \
        ...
        \n\tComponents pooled -> %lu\n", scene->Tag, scene, scene->Pool.EntityPooledCount, scene->Pool.QueueCount, scene->Pool.ComponentTableCount, scene->Pool.EntityPooledCount);
}
